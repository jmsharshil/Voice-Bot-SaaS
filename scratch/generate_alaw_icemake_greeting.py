...
def generate_alaw_greeting():
    ...
    alaw_bytes = res.content
    # Prepend 500ms of G.711 A-law silence (4000 bytes at 8000Hz) so telecom gateway does not cut off the first word
    silence_padding = b'\xd5' * 4000
    alaw_bytes = silence_padding + alaw_bytes
    ...
    for path in [out_file1, out_file2, out_file3, out_file4]:
        with open(path, "wb") as f:
            f.write(alaw_bytes)
        print(f"✅ Saved native A-law audio to: {path} ({len(alaw_bytes)} bytes)")

if __name__ == "__main__":
    generate_alaw_greeting()
