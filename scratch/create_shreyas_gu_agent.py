# scratch/create_shreyas_gu_agent.py
...
def setup_shreyas_gu():
    ...
    if created:
        print("Created new AgentRoleTemplate: Shreyas Sports Advisor Gujarati")
    else:
        # Update template prompt just in case it changed
        role_template.system_prompt_template = SHREYAS_GU_SYSTEM_PROMPT
        role_template.description = "Shreyas Foundation Sports & Outreach Programs Advisor in Gujarati"
        role_template.save()
        print("Updated existing AgentRoleTemplate: Shreyas Sports Advisor Gujarati")
    ...
    agent, created = VoiceAgent.objects.get_or_create(
        ...
        defaults={
            ...
            "summary": "Voice bot for Shreyas Foundation Sports & Outreach Programs in Gujarati.",
            "is_active": True
        }
    )
    if created:
        ...
        agent.summary = "Voice bot for Shreyas Foundation Sports & Outreach Programs in Gujarati."
        agent.save()
        print(f"Updated existing VoiceAgent: {agent.name} (ID: {agent.id})")

    print("\n--- Shreyas Gujarati Agent Setup Completed ---")
