...
class VoiceBotConsumer(AsyncWebsocketConsumer):
    ...
    async def connect(self):
        ...
        self.SILENCE_TRIGGER_SEC = 0.4   # 400ms — fires VAD dispatch faster after user stops talking
        self.MIN_WORD_COUNT = 1
    ...
    def _set_language(self, lang_code):
        # ⚡ IMPROVED: Avoid frequent/incorrect switching for short Hinglish phrases
        # If text is primarily Roman, and detected as 'gu', it's likely a mistake if previous was 'hi'/'en'
        if self.language in ["hi", "en"] and lang_code == "gu":
            # Check if partial text has Hindi/English characteristics? 
            # For now, let's just be more conservative if we are in a steady state.
            pass
    ...
    async def handle_ai_reply(self, text):
        ...
        _, prep_result = await asyncio.gather(
            save_message(self.conversation, "user", text),
            sync_to_async(prepare_streaming)(self.agent_id, text, self.session_id,detected_language=self.language)
        )
        prep_ms = round((time.time() - t_prep) * 1000)
    ...
    async def _synthesize_and_cache_greeting(self, text: str, tts_language: str = None):
        """First-call: synthesize greeting, cache audio, then stream it."""
        lang = tts_language or self.language
        loop = asyncio.get_event_loop()
        try:
            ulaw = await asyncio.wait_for(
