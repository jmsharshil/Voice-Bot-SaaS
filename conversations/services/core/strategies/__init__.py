...
Reply in the same language as the user."""
    ...
    # Splitting the user's negative responses into 'busy' vs 'not interested'
    elif any(kw in msg for kw in ["busy", "baad mein", "abhi nahi", "vyast", "kaam", "postpone", "meeting", "driving", "बाद में", "व्यस्त", "काम में हूँ", "अभी नहीं", "बादमे", "પછી વાત કરીએ", "કામમાં છું"]) and not mp3_phase:
        state["call_phase"] = "callback_request"
        save_session(session, state)
        system_prompt += """
⚠️ CURRENT PHASE: CALLBACK REQUEST
The user indicated they are busy or can't talk right now.
...
Reply in the same language as the user."""
    ...
    elif any(kw in msg for kw in ["nahi chahiye", "not interested", "no thanks", "nahi", "galat number", "wrong number", "nahi ki thi", "enquiry nahi", "already bought", "khareed li", "le li", "नहीं", "नहीं चाहिए", "ना", "बात नहीं करनी", "गलत नंबर", "नही", "જી નહીં", "નથી લેવી", "નથી જોઈતી", "નહીં જોઈએ", "રહેવા દો", "નથી", "ના", "ખરીદ લીધી"]) and not mp3_phase:
        state["call_phase"] = "denial_followup"
        save_session(session, state)
        system_prompt += """
⚠️ CURRENT PHASE: NOT INTERESTED / DENIAL
The user has indicated they don't want the car, are not interested, or bought a different car.
1. Politely ask them for the reason why they are not interested (e.g., "Oh! Koi specific reason ki aap abhi test drive nahi lena chahte?").
2. Do NOT add the [NOT_INTERESTED] tag yet.
3. Keep it to ONE short question.
Reply in the same language as user."""
...
# --- HOSPITAL MINIMAL STRATEGY ---
from .hospital_minimal import (
